...
// Test each weak transition individually
const weakTransitions = ['41->41', '44->43', '43->43', '42->42'];

for (const targetTrans of weakTransitions) {
  let baselineHits = 0;
  let overrideHits = 0;
  let totalRolls = 0;
  ...
  const delta = overrideHits - baselineHits;
  console.log(`${targetTrans}: baseline=${baselineHits} override=${overrideHits} delta=${delta > 0 ? '+' : ''}${delta}`);
}
...
console.log('\nALL 4 TOGETHER:');
{
  ...
  for (const file of files) {
    ...
    for (const session of sessions) {
      for (const row of session) {
        ...
        if (weakTransitions.includes(key)) {
          const ranked = result.analyzerFinalScores || [];
          const filtered = ranked.filter(e => e.value !== lastRoll);
          if (filtered.length >= 2) {
            newPred = filtered[0].value;
  ...
  const delta = overrideHits - baselineHits;
  console.log(`baseline=${baselineHits} override=${overrideHits} delta=${delta > 0 ? '+' : ''}${delta}`);
}
