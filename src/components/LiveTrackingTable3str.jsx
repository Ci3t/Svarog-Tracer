// src/components/LiveTrackingTable3str.jsx
...
export function LiveTrackingTable3str({ rolls = [] }) {
  if (rolls.length < 6) {
    return (
  ...
  const beastAnalysis = predictNext3BBPMode(rollValues);
  ...
  const distribution = Object.entries(freq).map(([value, count]) => {
    ...
    return {
      ...
      status,
      last5Pattern,
      // For sorting: commons first, then by count
      sortKey: status === 'common' ? 1000 + count : count
    };
  });
      ...
      <table className="freq-table">
        ...
        <tbody>
          {distribution.map((item) => (
            <tr key={item.value} className={item.status === 'common' ? 'is-common' : ''}>
...
export default LiveTrackingTable3str;
