      ...
      {testRolls.length > 0 ? (
        <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-sm font-mono text-purple-200">
                        {isSame ? translated : <><span className="text-slate-400">{raw}</span> <span className="text-purple-400">→</span> {translated}</>}
                      </div>
                      {!isSame && <div className="text-[10px] text-slate-500">raw → translated</div>}
                    </div>
                  </div>
                  <button
          ...
          <div className="pt-2 border-t border-slate-700">
            <div className="text-xs text-slate-400 mb-2">Quick prefix:</div>
            <div className="flex flex-wrap gap-2">
              {Array.from(new Set(translatedTestRolls.map((r) => r.slice(0, 2)))).map((prefix) => (
                <button
                  key={prefix}
        ...
        <div className="text-slate-500 italic p-4 text-center text-sm">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div className="text-left">
            <div className="text-sm font-bold text-purple-300">Added Rolls</div>
            <div className="text-xs text-slate-400">
              {testRolls.length} rolls • Click to {isCollapsed ? "expand" : "collapse"}
            </div>
          </div>
        </div>
        <span className="text-slate-400 cursor-pointer">{isCollapsed ? "▼" : "▲"}</span>
      </button>
